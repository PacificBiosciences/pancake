...
TEST(DPChainSimd, GroupByTargetAndStrand_ArrayOfTests)
{
    struct TestData
    {
        std::string testName;
...
TEST(DPChainSimd, DiagonalGroup_ArrayOfTests)
{
    struct TestData
    {
        std::string testName;
...
///////////////
        {
            ...
            // Seed hits.
            {
                ...
                {0, 0, 5000, 5200, 15, 15, 0},  // 12 -> SIMD specific: This is the last one checked from seed 19, because the distance is a multiple of 4.
    ...
    for (const auto& data : allTests) {
        ...
        std::vector<ChainedHits> results = ChainHitsSimd(
