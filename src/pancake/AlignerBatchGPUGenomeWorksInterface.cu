...
__global__ void convert_to_pacbio_kernel(PacBio::Data::CigarOperation* cigar, int4* diffs,
                                         ...
                                         const int32_t* cigar_offsets, const uint32_t* metadata,
                                         const int8_t* cigar_operations,
                                         const int32_t* cigar_runlengths, int64_t cigar_buf_size,
                                         int32_t start_offset_index, int32_t n_alignments,
                                         ...
                                         int32_t gap_open_penalty, int32_t gap_ext_penalty)
{
    ...
    const int32_t offset_idx =
        upper_bound(cigar_offsets + start_offset_index, cigar_offsets + n_alignments, i) -
        cigar_offsets - 1;
    assert(offset_idx >= 0);
    ...
    if ((start + len) > (global_start + cigar_buf_size)) return;
...
void RunConvertToPacBioCigarAndScoreGpuKernel(
    ...
    int64_t cigar_buf_size, int32_t start_at_offset_index, int32_t match_score,
    int32_t mismatch_score, int32_t gap_open_score, int32_t gap_ext_score, cudaStream_t stream,
    int32_t device_id)
{
    ...
    convert_to_pacbio_kernel<<<n_blocks, n_threads, 0, stream>>>(
        ...
        aln_ptrs.cigar_offsets, aln_ptrs.metadata, aln_ptrs.cigar_operations,
        aln_ptrs.cigar_runlengths, cigar_buf_size, start_at_offset_index, n_alignments, match_score,
        mismatch_score, gap_open_score, gap_ext_score);
