...
/**
     ...
     */
inline char ksw2c_get_cigar_op_type(ksw2c_cigar_op cigar_op) { return "MIDU"[cigar_op & 0x3]; }
...
/**
     ...
     */
inline uint32_t ksw2c_get_cigar_op_length(ksw2c_cigar_op cigar_op) { return cigar_op >> 4; }
...
std::vector<ksw2c_response> ksw2c_batch(
    ...
    int8_t start_gap_cost, int8_t extend_gap_cost, int8_t start_gap_cost_2,
    int8_t extend_gap_cost_2, const std::vector<int8_t> scoring_matrix, int32_t bandwidth,
    int32_t zdrop, int32_t end_bonus, bool extension_only_cigar);
}  // namespace hugenomic
